...
def audit(root: Path) -> list[str]:
    ...
    if not workflows_dir.is_dir():
        ...
        # The owner explicitly authorized disposable PR #15/#16, #19 and #20
        # previews for first-access and Turmas testing. Reject unscoped distribution, production binaries,
        ...
        if distributes:
            required = (
                "github.event_name == 'pull_request' && (github.event.pull_request.number == 15 || github.event.pull_request.number == 16 || github.event.pull_request.number == 19 || github.event.pull_request.number == 20)",
