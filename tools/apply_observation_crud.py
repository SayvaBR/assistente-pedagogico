...
tests = Path('app/src/androidTest/java/com/sayvabr/assistentepedagogico/data/TeacherStoreInstrumentedTest.kt')
s, t, q = store.read_text(), ui.read_text(), tests.read_text()
...
s = rep(s, '    fun addAppointment(title: String, day: String, time: String) {', '''    /** Edits an existing pedagogical observation without changing its identity or original date. */
    ...
    fun addAppointment(title: String, day: String, time: String) {''', 'observation store CRUD')

t = rep(t, '    var selectedLesson by rememberSaveable { mutableLongStateOf(-1L) }\n', '    var selectedLesson by rememberSaveable { mutableLongStateOf(-1L) }\n    var selectedObservation by rememberSaveable { mutableLongStateOf(-1L) }\n', 'selected observation state')
t = rep(t, '''          "classDetail" -> if (currentClass != null) ClassDetail(snapshot, currentClass, { navigate(it) },
...
''', 'class detail observation navigation')
t = rep(t, '''                    "observation" -> if (currentClass != null) ObservationForm(snapshot, currentClass, { back() }, { studentId, kind, body, share -> commit("classDetail") { store.addObservation(currentClass.id, studentId, kind, body, share) } })
...
t = rep(t, '"newLesson", "editLesson", "observation", "newAppointment"', '"newLesson", "editLesson", "observation", "editObservation", "newAppointment"', 'hide tabs while editing observation')
t = rep(t, '@Composable private fun ClassDetail(data: TeacherSnapshot, classroom: Classroom, go: (String) -> Unit, onStudent: (Long) -> Unit) {', '@Composable private fun ClassDetail(data: TeacherSnapshot, classroom: Classroom, go: (String) -> Unit, onStudent: (Long) -> Unit, onObservation: (Long) -> Unit) {', 'class detail signature')
t = rep(t, '''    data.observations.filter { it.classroomId == classroom.id }.take(5).forEach { note ->
        Panel {
        ...
        Spacer(Modifier.height(8.dp))
    }''', 'actionable recent observations')

start = t.index('@Composable private fun ObservationForm(')
end = t.index('\n@Composable private fun PlanningScreen', start)
t = t[:start] + '''@Composable private fun ObservationForm(data: TeacherSnapshot, classroom: Classroom, back: () -> Unit,
...
''' + t[end:]
print('PATCH observation form edit/delete')

anchor = '    @Test fun invalidAttendanceBatchRollsBackAllMarks() {'
test = '''    @Test fun observationEditAndDeleteAreScopedAndPersist() {
        val first = db()
...
'''
q = rep(q, anchor, test + anchor, 'observation CRUD persistence test')
store.write_text(s)
ui.write_text(t)
tests.write_text(q)
