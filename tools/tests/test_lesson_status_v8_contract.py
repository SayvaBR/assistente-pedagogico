...
class LessonStatusV8ContractTest(unittest.TestCase):
    def test_versioned_additive_schema_and_observable_store_snapshot(self):
        ...
        layout = (DATA / "PlanLayoutV9.kt").read_text(encoding="utf-8")
        ...
        self.assertIn('"pedagogico.db", null, PlanLayoutV9.VERSION', store)
        self.assertIn('oldVersion in 1..8', store)
        ...
        self.assertIn('PlanLayoutV9.migrate(db)', store)
        self.assertIn('CREATE TABLE IF NOT EXISTS lesson_layouts', layout)
        self.assertIn('CREATE TABLE IF NOT EXISTS plan_templates', layout)
